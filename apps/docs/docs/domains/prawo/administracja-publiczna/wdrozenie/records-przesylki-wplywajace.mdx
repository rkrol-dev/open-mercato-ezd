---
title: Analiza wdrożeniowa - Records (Przesyłka wpływająca)
sidebar_position: 10
---
...
## Stan implementacji (na dzień 2026-01-26)

### ✅ Feature Toggles (Mechanizm włączania/wyłączania funkcjonalności)

Wszystkie nowe moduły są zabezpieczone mechanizmem feature toggles, który pozwala na:
- **Kontrolowane włączanie funkcjonalności** na poziomie globalnym lub per-tenant
- **Bezpieczne wdrożenie** stopniowe (rollout per organizacja)
- **Wyłączanie w nagłych przypadkach** (emergency kill switch)
- **Testowanie na środowiskach produkcyjnych** bez wpływu na użytkowników końcowych

#### Zdefiniowane feature flags:

1. **`records_incoming_shipments`** (boolean, default: `false`)
   - **Nazwa**: Records - Incoming Shipments
   - **Opis**: Włącza moduł przesyłek wpływających z rejestracją RPW i zarządzaniem dokumentami
   - **Kategoria**: `records`
   - **Wpływ**: Ukrywa/pokazuje wszystkie funkcje związane z przesyłkami wpływającymi w interfejsie użytkownika
   - **Ekrany**: Lista przesyłek, formularz create/edit, rejestracja RPW, szczegóły z dokumentami

2. **`records_jrwa_classes`** (boolean, default: `false`)
   - **Nazwa**: Records - JRWA Classes
   - **Opis**: Włącza system klasyfikacji JRWA z importem CSV i politykami retencji
   - **Kategoria**: `records`
   - **Wpływ**: Ukrywa/pokazuje funkcje klasyfikacji rzeczowej akt
   - **Ekrany**: Lista klas JRWA, formularz create/edit, wizard importu CSV

3. **`correspondence_sources`** (boolean, default: `false`)
   - **Nazwa**: Correspondence Sources Integration
   - **Opis**: Włącza moduł integracji źródeł korespondencji (eDoreczenia, ePUAP) z automatycznym tworzeniem przesyłek
   - **Kategoria**: `integrations`
   - **Wpływ**: Ukrywa/pokazuje konfigurację źródeł korespondencji i synchronizację
   - **Ekrany**: Lista źródeł, formularz konfiguracji, logi synchronizacji

4. **`correspondence_sources_mock_ui`** (boolean, default: `false`)
   - **Nazwa**: Mock eDoreczenia Interface
   - **Opis**: Włącza interfejs administratora do testowania synchronizacji korespondencji (superadmin only)
   - **Kategoria**: `integrations`
   - **Wpływ**: Ukrywa/pokazuje mock eDoreczenia do testów
   - **Ekrany**: Lista mock correspondence (tylko dla superadmin)

#### Implementacja techniczna:

**Frontend (React):**
```tsx
// Automatyczne ukrywanie komponentów gdy feature wyłączony
import { FeatureGuard } from '@open-mercato/core/modules/feature_toggles/components/FeatureGuard'

<FeatureGuard
  id="records_incoming_shipments"
  fallback={<Alert>Feature is disabled</Alert>}
>
...
const result = await featureTogglesService.getBoolConfig('records_incoming_shipments', tenantId)
...
**Konfiguracja:**
- Domyślne wartości: `packages/core/src/modules/feature_toggles/defaults.json`
- Zarządzanie: Backend → Feature Toggles → Global lub per-tenant overrides
- CLI: `yarn mercato feature-toggles:toggle-create` / `toggle-update` / `override-set`

**Caching:**
- Resolutions są cache'owane z TTL 1 minuty
- Invalidacja po zmianie wartości (natychmiastowe zastosowanie)
- Cache tags: `feature_toggles:identifier:{flag}` i `feature_toggles:tenant:{tenantId}`

#### Instrukcje aktywacji:

Zobacz sekcję **"Aktywacja modułów za pomocą feature toggles"** w dokumentacji użytkownika.

### ✅ Zaimplementowane (Backend/API)

#### Encje i model danych
- **`RecordsIncomingShipment`** (packages/core/src/modules/records/entities/RecordsIncomingShipment.entity.ts)
  - Pełny model z wszystkimi polami zgodnie z sekcją 5.1
  - Relacje do organization, tenant, receiving org unit
  - Obsługa statusów: `draft`, `registered`
  - Pola metadanych: subject, sender, delivery method, received_at
  - RPW number generation z wykorzystaniem `RecordsRpwSequence`
  - Document IDs jako array UUID (relacja do przyszłej encji `RecordsDocument`)
  
- **`RecordsRpwSequence`** (packages/core/src/modules/records/entities/RecordsRpwSequence.entity.ts)
  - Generator sekwencji numerów RPW
  - Scope: organization + receiving org unit symbol + year
  - Automatyczne resetowanie sekwencji na początek roku
  - Thread-safe increment operations

- **`RecordsJrwaClass`** (packages/core/src/modules/records/entities/RecordsJrwaClass.entity.ts)
  - Hierarchiczna struktura klas rzeczowych (parent-child)
  - Pola: code, name, description, retention_category, archival_package_variant
  - Wersjonowanie (`version`, `is_active`)
  - Obsługa importu CSV z zachowaniem historii

#### API Routes (CRUD + akcje procesowe)
- **`/api/records/incoming-shipments`** (packages/core/src/modules/records/api/incoming-shipments/)
  - ✅ GET (list z filtrowaniem, sortowaniem, paginacją)
  - ✅ POST (create z walidacją Zod)
  - ✅ PUT (update z immutability RPW)
  - ✅ DELETE (soft delete)
  - ✅ POST `/:id/register` (akcja procesowa - nadanie RPW)
  
- **`/api/records/jrwa-classes`** (packages/core/src/modules/records/api/jrwa-classes/)
  - ✅ GET (list z hierarchią)
  - ✅ POST (create)
  - ✅ PUT (update)
  - ✅ DELETE
  - ✅ POST `/import` (import CSV z walidacją i wersjonowaniem)

#### Walidacja (Zod schemas)
- **IncomingShipmentSchemas** (packages/core/src/modules/records/api/incoming-shipments/schemas.ts)
  - Walidacja create/update
  - Reguły biznesowe: wymagane pola dla rejestracji, immutability RPW
  - Custom validations: sender (senderId XOR senderDisplayName)
  
- **JrwaClassSchemas** (packages/core/src/modules/records/api/jrwa-classes/schemas.ts)
  - Walidacja struktury hierarchicznej
  - Import CSV validation

#### OpenAPI
- ✅ Wszystkie routes mają eksportowany `openApi` object
- ✅ Dokumentacja request/response dla każdego endpointu
- ✅ Modułowy helper `createRecordsCrudOpenApi()` (analogicznie do sales/catalog)

#### Uprawnienia (ACL)
- ✅ Feature gates zdefiniowane zgodnie z sekcją 7:
...
#### Search (Cmd+K)
- ✅ `formatResult()` dla incoming shipments (wyświetla: subject + RPW)
- ✅ `formatResult()` dla JRWA classes (wyświetla: code — name)
- ✅ Tokenizacja pól: subject, sender_name, rpw_number (shipments); code, name (JRWA)

## Integracja z modułem correspondence_sources (automatyczne pobieranie korespondencji)
- ✅ **Moduł `correspondence_sources`** (packages/core/src/modules/correspondence_sources/)
  - Implementacja multi-source pattern dla automatycznego pobierania korespondencji z systemów zewnętrznych
  - Obsługa eDoreczenia (mock), ePUAP, email i innych źródeł
  - **Automatyczne tworzenie przesyłek wpływających** (`RecordsIncomingShipment`) **w statusie `draft`** - wymaga weryfikacji użytkownika przed rejestracją
  - **Mapowanie nadawców**: Próba dopasowania istniejącego klienta po znormalizowanych NIP/REGON/PESEL/KRS, tworzenie nowego tylko przy braku dopasowania
  - Mapowanie metadanych: subject, sender, received_at, delivery_method (= source type)
  - **Pole `delivery_method`**: automatycznie ustawiane na typ źródła (np. "edoreczenia-mock", "epuap")
  - **Obsługa załączników**: Automatyczne tworzenie dokumentów (`records_documents`) dla załączników (Phase 2)
  - **Częstotliwość pobierania**: 
    - Manual: Przycisk "Sprawdź nowe" (na żądanie użytkownika)
    - Scheduled: Automatycznie co 15 minut (konfigurowalny przez Admina organizacji)
  - **Potwierdzenie odbioru (UPD)**: Automatyczne wysyłanie potwierdzenia odbioru do nadawcy (wymóg prawny)
  - Audit trail: logi synchronizacji, śledzenie pomyślnych/niepomyślnych operacji
  - Szczegóły: [Dokumentacja implementacji integracji eDoreczenia](/docs/integracje/edoreczenia/implementation)
  - **Kluczowe ustalenia biznesowe**: [Ustalenia z wdrożenia integracji - Sekcja 11](/docs/integracje/edoreczenia/ustalenia-wdrozenia#11-kluczowe-ustalenia-biznesowe-integracji-edoreczenia)

### ❌ Niezaimplementowane (Frontend/UI)

**Brak jakichkolwiek komponentów UI** dla modułu `records`. Wymagana pełna implementacja zgodnie z ustaleniami w sekcji D) UI i UX oraz wzorcami z istniejących modułów (szczegóły w sekcji 13A poniżej).

Komponenty do zaimplementowania:
1. **Lista przesyłek wpływających** - DataTable z filtrowaniem i sortowaniem
2. **Formularz create/edit przesyłki** - CrudForm z sekcjami
3. **Szczegóły przesyłki** - layout z sekcjami (metadane, dokumenty, historia)
4. **Dokumenty na przesyłce** - embedded ItemsSection-like component
5. **JRWA - lista/drzewo klas** - hierarchiczna prezentacja
6. **JRWA - import CSV** - dedykowana strona z podglądem i walidacją
7. **Akcja "Zarejestruj wpływ"** - przycisk z walidacją i feedback

### 🔄 W trakcie / wymagane doprecyzowania

- **Encja `RecordsDocument`**: częściowo zdefiniowana w sekcji 5.3, ale nie zaimplementowana
- **Attachments/Files**: mechanizm storage i upload (powiązanie z encją Document)
- **Audit log**: mechanizm logowania zmian (timeline/historia)
- **Integracja z org structure**: pobieranie `receiving_org_unit_symbol` z modułu organizational
- **Generator numerów**: implementacja thread-safe sequence generator dla RPW
...
```mermaid
graph TD
    A[Start: użytkownik w module Records] --> B{Ma uprawnienie<br/>manage?}
    B -->|Nie| C[Brak przycisku Nowa przesyłka]
    B -->|Tak| D[Klik: Nowa przesyłka]
    D --> E[Strona: /records/incoming-shipments/new]
    E --> F[Formularz w statusie draft]
    F --> G[Wypełnienie: Temat, Nadawca, Sposób wpływu]
    G --> H[Wybór: Kancelaria lookup organizational]
    H --> I[Wybór: Data wpływu calendar]
    I --> J[Klik: Dodaj dokument]
    J --> K[Dialog: nowy dokument]
    K --> L[Wypełnienie: Tytuł dokumentu, Typ]
    L --> M[Optional: Upload załączników]
    M --> N[Klik: Zapisz dokument]
    N --> O[Dokument dodany do listy]
    O --> P{Więcej dokumentów?}
    P -->|Tak| J
    P -->|Nie| Q[Klik: Zapisz przesyłkę]
    Q --> R[Walidacja: pola podstawowe OK]
    R --> S[API: POST /api/records/incoming-shipments]
    S --> T[Sukces: przesyłka zapisana w draft]
    T --> U{Czy rejestrować wpływ?}
    U -->|Nie| V[Redirect: lista przesyłek]
    U -->|Tak| W[Przycisk: Zarejestruj wpływ widoczny]
    W --> X[Walidacja: dokumenty niepuste + wszystkie pola]
    X -->|Błąd| Y[Komunikat: uzupełnij wymagane]
    X -->|OK| Z[Klik: Zarejestruj wpływ]
    Z --> AA[API: POST /api/.../id/register]
    AA --> AB[Sukces: nadanie RPW]
    AB --> AC[Flash: Przesyłka zarejestrowana RPW/XXX/00001/2026]
    AC --> AD[Status zmieniony: registered]
    AD --> AE[RPW visible, immutable]
    AE --> AF[Redirect: szczegóły przesyłki]
    AF --> AG[End]
```
...
graph TD
    ...
    G --> H[Lista: ikona inbox + tytuł subject + subtitle RPW]
    H --> I[Klik: wynik]
